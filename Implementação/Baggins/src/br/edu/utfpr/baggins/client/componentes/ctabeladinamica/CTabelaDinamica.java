...
public abstract class CTabelaDinamica
{
	...
	private void setarInformacoes()
	{
		...
		btnAdicionar.setActionExpression(CControleTabelaDinamica.criasMethodExpression("#{tabelaDinamicaBean.botaoAdicionar}", Object.class));

		btnCancelar.setActionExpression(CControleTabelaDinamica.criasMethodExpression("#{tabelaDinamicaBean.botaoCancelar}", Object.class));

		btnExcluir.setActionExpression(CControleTabelaDinamica.criasMethodExpression("#{tabelaDinamicaBean.botaoExcluir}", Object.class));
	...
	private void gerarPesquisa()
	{
		...
		btnBuscar.setActionExpression(CControleTabelaDinamica.criasMethodExpression("#{tabelaDinamicaBean.botaoBuscar}", Object.class));
	...
	private void controleGrid()
	{
		...
		ValueExpression valueArr = ef.createValueExpression(elc, "#{tabelaDinamicaBean.model}", Object.class);
		grid.setValueExpression("value", valueArr);

		ValueExpression valueSele = ef.createValueExpression(elc, "#{tabelaDinamicaBean.objetoSelecionado}", Object.class);

		grid.setValueExpression("selection", valueSele);

		ValueExpression valueRow = ef.createValueExpression(elc, "#{item.id}", Object.class);
		...
		MethodExpression me = ef.createMethodExpression(elc, "#{tabelaDinamicaBean.onRowSelect}", Void.class, new Class<?>[]
		{});
		...
		for (int i = 0; i < TabelaDinamicaBean.getNomeColunasMap().size(); i++)
		{
			try
			{
				...
				HtmlOutputText saida = (HtmlOutputText) application.createComponent(HtmlOutputText.COMPONENT_TYPE);

				ValueExpression nameValueExp = ef.createValueExpression(elc, "#{item." + TabelaDinamicaBean.getNomeColunasMap().get(i) + "}", Object.class);
				saida.setValueExpression("value", nameValueExp);
				colunsMap.get(TabelaDinamicaBean.getNomeColunasMap().get(i)).getChildren().add(saida);
			}
			catch (Exception e)
			{
				e.printStackTrace();
			...
			for (Annotation anotation : field.getDeclaredAnnotations())
			{
				if (anotation instanceof TabelaDinamica)
				{
					...
					if (tab.grid())
					{
						...
						TabelaDinamicaBean.getNomeColunasMap().put(colunaAcres, field.getName());
			...
			for (Field field : getFieldObjeto())
			{
				...
				if (nome != null)
				{
					if (!field.getType().isPrimitive() && !field.getType().isAssignableFrom(Date.class) && !field.getType().isAssignableFrom(String.class))
					{
						...
						TabelaDinamicaBean.setClasseCombo(limparNomeClasse(field.getType().toString()));

						for (Annotation ano : field.getDeclaredAnnotations())
						{
							if (ano instanceof TabelaDinamica)
							{
								if (!((TabelaDinamica) ano).whereComboBox().isEmpty())
								{
									TabelaDinamicaBean.setWhereCombo(((TabelaDinamica) ano).whereComboBox());
								}
							}
						}
						...
						selectItem.setValueExpression("value", CControleTabelaDinamica.criarValueExpression("#{tabelaDinamicaBean.listComboBox['" + nome + "']}", Object.class));

						campoNovo.setValueExpression("value", CControleTabelaDinamica.criarValueExpression("#{tabelaDinamicaBean.mapaComboSelecionado['" + field.getName() + "']}", String.class));

						if (i == 0)
						{
							campoNovo.setValueExpression("style", CControleTabelaDinamica.criarValueExpression("width:100%;", String.class));
						}
						else
						{
							campoNovo.setValueExpression("style", CControleTabelaDinamica.criarValueExpression("width:100%; margin-right: 22px;", String.class));
						}
						i++;
					...
					TabelaDinamicaBean.getInputsMap().put(field.getName(), campoNovo);
	...
	public List<Object> buscarArrayInicial()
	{
		return (List<Object>) CControleDinamicoPU.getListBuscaGenericaaa(objetoClass, "");
	}
